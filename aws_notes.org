...
- Partition: groups of instances spread apart to reduce the likelihood of correlated failures for large distributed and replicated workloads.
  - Maximum of 7 partitions  per AZ - each can  many instances.
...
- Asynchronous invocation: Lambda stores the event in an internal queue and handles the invocation.
  ...
  - If the function fails, it will retry between 0 and 2 times (configurable).
    - Events can be sent to a dead letter queue after repeated failures in processing. or destinations ( newer - failure and success) 
...
- io1, io2, io2 Block Express:
  ...
  - io1	64,000	1,000 MB/s	Older generation provisioned IOPS.
  io2	256,000	4,000 MB/s	Higher durability (99.999%). Better price/perf than io1.
  io2 Block Express	256,000	up to 64,000 MB/s	Next-gen architecture. Very low latency (<1 ms). Supports volumes up to 64 TiB.
...
- Security Groups:
  - DB Security Groups: controls access to a DB instance that is not in a VPC. By default, network access is turned off to a DB instance. ( OLD - NOT USED ANYMORE )
